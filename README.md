...
| ローカル KVM | Samba/GVFS 共有 | sudo rsync → /tmp → shutil |
...
2. `rsync` と `tee` の passwordless sudo 設定 (`/etc/sudoers.d/kvm-migrate-rsync`)
3. Python 仮想環境 (`.venv`) の作成
4. Python パッケージのインストール (`PyQt6`, `libvirt-python`)

> **注意**: `/var/lib/libvirt/images/` は root 所有のため、ディスク転送に `sudo rsync` / `sudo tee` を使用します。
